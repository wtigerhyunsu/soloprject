...
<!--nice pay -->
	...
	function goPay(payForm) {
		...
		if (hasFrame) {
			documentBody = parent.document.body
		...
		try {
			if (!document.body) {
				...
				NicePayCommon.errorReport("MJ04",
						"[MerchantJS]Merchant access fail document body");
				return false;
			}

			if (payForm.encoding.indexOf("multipart") > -1) {
				alert("지원하지 않는 방식으로 결제창이 요청되었습니다. 가맹점으로 문의 해주시기 바랍니다.");
				NicePayCommon.errorReport("MJ01",
						"[MerchantJS]Form encoding requested in multipart");
				return false;
			}
			...
			if (browser.msie && browser.version < 9) {
				...
				NicePayCommon.errorReport("MJ02",
						"[MerchantJS]Payment requested on IE9 or lower");
				return false;
			}

			if (browser.msedge) {
				// edge confirm check
				if (payForm.DisableEdgeChk
						&& payForm.DisableEdgeChk.value == "Y") {
					// do nothing ~
				} else {
					if (browser.version < 17) {
						...
						if (confirm(str_comfirm)) {
							window.open("about:flags", "optionWindow",
			...
			var NpDirectCard = [ [ "02", "459", "456", "" ],
					[ "03", "644", "480", "" ], [ "04", "400", "400", "" ],
					[ "06", "400", "400", "" ], [ "07", "400", "400", "" ],
					[ "08", "644", "489", "" ], [ "11", "644", "480", "" ],
					[ "12", "400", "400", "" ], [ "15", "644", "510", "" ],
					[ "16", "644", "480", "" ], [ "37", "500", "456", "" ],
					[ "SSG", "640", "685", "" ],
					[ "SSG_MONEY", "640", "685", "" ],
			...
			var NpDirectBank = [ [ "LIIV", "700", "665", "" ],
					[ "BANK", "0", "0", "POPUP" ] ];

			var NpDirectCellPhone = [ [ "CELLPHONE", "0", "0", "POPUP" ] ];

			var NpDirectSsgBank = [ [ "SSG_BANK", "0", "0", "POPUP" ] ];

			var NpDirectCmsBank = [ [ "CMS_BANK", "450", "670", "" ] ];
				...
				if (DirectShowArray.length > 0 && DirectShowArray.length < 10) {
					for (var i = 0; i < DirectShowArray.length; i++) {
						if (DirectShowArray[i] == PayMethodValue) {//CARD, BANK, CELLPHONE, SSG_BANK, CMS_BANK
							...
							if (DirectShowArray[i] == "GIFT_SSG"
									&& PayMethodValue == "CARD"
									&& SelectCardCode.value == "SSG_MONEY") {
								DirectFlag = true;
								DriectCardCd = SelectCardCode.value;
								break;
							}
			...
			if (NicepayReserved) {
				...
				if (NiceReservedArr.length > 0) {
					for (var i = 0; i < NiceReservedArr.length; i++) {
						...
						if (ReservedParamArr[0] == "DirectSamsungPay"
								&& ReservedParamArr[1] == "Y") {
							DriectCardCd = "SamsungPay";
							break;
						}
						if (ReservedParamArr[0] == "DirectPayco"
								&& ReservedParamArr[1] == "Y") {
							DriectCardCd = "Payco";
							break;
						}
						if (ReservedParamArr[0] == "DirectKakao"
								&& ReservedParamArr[1] == "Y") {
							DriectCardCd = "KakaoPay";
							break;
						}
						if (ReservedParamArr[0] == "DirectPay11"
								&& ReservedParamArr[1] == "Y") {
							DriectCardCd = "SKPay";
							break;
						}
			...
			if (DirectFlag) {
				if (PayMethodValue == "CARD") {
					...
					for (var i = 0; i < NpDirectCellPhone.length; i++) {
						if (NpDirectCellPhone[i][0] == "CELLPHONE") {
			...
			NicePayCommon.setFormData(niceForm, "EncGoodsName",
					escape(niceForm.GoodsName.value));
			NicePayCommon.setFormData(niceForm, "EncBuyerName",
					(niceForm.BuyerName) ? escape(niceForm.BuyerName.value)
							: "");
			NicePayCommon.setFormData(niceForm, "NpDirectYn",
					(DirectFlag == true) ? "Y" : "N");
			...
			NicePayCommon.errorReport("MJ03", "[MerchantJS]Exception : "
					+ e.message);
			return false;
		}
	}
	...
	NicePayCommon.createDivElement = function(frameID, zindex) {
		if (document.getElementById(frameID) == null) {
			var newDiv = (hasFrame == false) ? document.createElement('div')
	...
	NicePayCommon.errorReport = function(errorCd, errorMsg) {
		try {
			...
			setTimeout(function() {
				documentBody.removeChild(document
						.getElementById("error_report"));
				documentBody
						.removeChild(document.getElementById("error_frame"));
			}, 1000);
		} catch (e) {
		}
	}
	...
	NicePayStd.receiveMessageValue = function(e) {
		...
		if (window.postMessage) {
			...
			NicePayStd.resultMessage({
				"code" : "1",
				"message" : "postMessage not supported"
			});
		}
	}
	NicePayStd.resultMessage = function(obj) {
		...
		case "0":
			//success
			if (obj.result) {
				NicePayCommon.setFormData(merchantForm, "AuthResultCode",
						"0000");
				NicePayCommon.setFormData(merchantForm, "AuthResultMsg",
						"인증 성공");
				NicePayCommon.setFormData(merchantForm, "AuthToken",
						obj.result.AuthToken);
				NicePayCommon.setFormData(merchantForm, "TxTid",
						obj.result.TxTid);
				NicePayCommon.setFormData(merchantForm, "NextAppURL",
						obj.result.NextAppURL);
				NicePayCommon.setFormData(merchantForm, "NetCancelURL",
						obj.result.NetCancelURL);
				NicePayCommon.setFormData(merchantForm, "Signature",
				...
				nicepaySubmit();
				if (merchantForm.RmAuthLayer
						&& merchantForm.RmAuthLayer.value == "Y") {
					NicePayStd.deleteLayer();
				} else {
		...
		case "1":
			if (obj.result) {
				NicePayCommon.setFormData(merchantForm, "AuthResultCode",
						obj.result.AuthResultCode);
				NicePayCommon.setFormData(merchantForm, "AuthResultMsg",
						obj.result.AuthResultMsg);
				NicePayCommon.setFormData(merchantForm, "AuthToken",
						obj.result.AuthToken);
				NicePayCommon.setFormData(merchantForm, "Signature",
						obj.result.Signature);
		...
		case "2":
			...
			break;
		case "10":
			var payLayer = (hasFrame == false) ? document
					.getElementById("nice_layer") : parent.document
					.getElementById("nice_layer");
	...
	NicePayStd.creatLayer = function(w, h, opt) {
		...
		html += "	<iframe name=\"nice_frame\" id=\"nice_frame\" src=\"\" width=\"100%\" height=\""
				+ h + "px\" scrolling=\"no\" frameborder=\"no\"></iframe>";
		html += "</div>";

		var payLayer = (hasFrame == false) ? document
				.getElementById("nice_layer") : parent.document
				.getElementById("nice_layer");
		var bgLayer = (hasFrame == false) ? document.getElementById("bg_layer")
				: parent.document.getElementById("bg_layer");

		payLayer.innerHTML = html;
		if (payLayer != null) {
			payLayer.style.top = (NicePayStd.getWindowHeight() - h) / 2
					+ NicePayStd.getYposition() + "px";
			payLayer.style.left = (NicePayStd.getWindowWidth() - w) / 2
					+ NicePayStd.getXposition() + "px";
		}
		...
		try {
			if (merchantForm.OptionList != null
					&& merchantForm.OptionList != "undefined") {
	...
	NicePayStd.deleteLayer = function() {
		...
		if (hasFrame) {
			documentBody.removeChild(parent.document
					.getElementById("nice_layer"));
			documentBody
					.removeChild(parent.document.getElementById("bg_layer"));
		} else {
	...
	NicePayStd.deletePayment = function() {
		var nicelayer = (hasFrame == false) ? document
				.getElementById("nice_layer") : parent.document
				.getElementById("nice_layer");
		if (hasFrame) {
			nicelayer.removeChild(parent.document
					.getElementById("payment_layer"));
		} else {
	...
	NicePayStd.getYposition = function() {
		...
		if (hasFrame) {
			...
			if (typeof (window.pageYOffset) == 'number') {
				...
				scrollY = (hasFrame == false) ? document.body.scrollTop
						: parent.document.body.scrollTop;
			} else if (document.documentElement
					&& (document.documentElement.scrollTop)) {
				scrollY = (hasFrame == false) ? document.documentElement.scrollTop
						: parent.document.documentElement.scrollTop;
			}
		}
	...
	NicePayStd.getXposition = function() {
		...
		if (hasFrame) {
			...
			if (typeof (window.pageXOffset) == 'number') {
				...
				scrollX = (hasFrame == false) ? document.body.scrollLeft
						: parent.document.body.scrollLeft;
			} else if (document.documentElement
					&& (document.documentElement.scrollLeft)) {
				scrollX = (hasFrame == false) ? document.documentElement.scrollLeft
						: parent.document.documentElement.scrollLeft;
			}
		}
		return scrollX;
	...
	NicePayStd.niceLayerHandler = function() {
		try {
			var nicelayer = (hasFrame == false) ? document
					...
					.getElementById("nice_layer");
			if (nicelayer != null) {
				nicelayer.style.top = (NicePayStd.getWindowHeight() - nicelayer.style.height
						.replace("px", ""))
						/ 2 + NicePayStd.getYposition() + "px";
				nicelayer.style.left = (NicePayStd.getWindowWidth() - nicelayer.style.width
						.replace("px", ""))
						/ 2 + NicePayStd.getXposition() + "px";
			}
		} catch (e) {
		}
	}
		...
		if (hasFrame) {
			if (parent.window.addEventListener) { // all browsers except IE before version 9
				parent.window.addEventListener("message",
						NicePayStd.receiveMessageValue, false);
			} else {
				if (parent.window.attachEvent) { // IE before version 9
					parent.window.attachEvent("onmessage",
							NicePayStd.receiveMessageValue); // Internet Explorer from version 8
				}
			}
			if (typeof parent.window.addEventListener != "undefined")
				parent.window.addEventListener("resize",
						NicePayStd.niceLayerHandler, false);
			if (typeof parent.window.attachEvent != "undefined")
				parent.window.attachEvent("onresize",
			...
			if (typeof window.addEventListener != "undefined")
				window.addEventListener("resize", NicePayStd.niceLayerHandler,
						false);
			if (typeof window.attachEvent != "undefined")
				window.attachEvent("onresize", NicePayStd.niceLayerHandler);

			if (typeof window.addEventListener != "undefined")
				window.addEventListener("scroll", NicePayStd.niceLayerHandler,
						false);
			if (typeof window.attachEvent != "undefined")
				window.attachEvent("onscroll", NicePayStd.niceLayerHandler);
		}
	}
	...
	NicePayStd.removeListener = function() {
		try {
			if (hasFrame) {
				if (parent.window.addEventListener) { // all browsers except IE before version 9
					parent.window.removeEventListener("message",
							NicePayStd.receiveMessageValue, false);
				} else {
					if (parent.window.attachEvent) { // IE before version 9
						parent.window.detachEvent("onmessage",
								NicePayStd.receiveMessageValue); // Internet Explorer from version 8
					}
				}
				if (typeof parent.window.addEventListener != "undefined")
					parent.window.removeEventListener("resize",
							NicePayStd.niceLayerHandler, false);
				if (typeof parent.window.attachEvent != "undefined")
					parent.window.detachEvent("onresize",
							NicePayStd.niceLayerHandler);
				...
				if (typeof parent.window.addEventListener != "undefined")
					parent.window.removeEventListener("scroll",
							NicePayStd.niceLayerHandler, false);
				if (typeof parent.window.attachEvent != "undefined")
					parent.window.detachEvent("onscroll",
							NicePayStd.niceLayerHandler);
			} else {
				if (window.addEventListener) { // all browsers except IE before version 9
				...
				if (typeof window.addEventListener != "undefined")
					window.removeEventListener("resize",
							NicePayStd.niceLayerHandler, false);
				if (typeof window.attachEvent != "undefined")
					window.detachEvent("onresize", NicePayStd.niceLayerHandler);

				if (typeof window.addEventListener != "undefined")
					window.removeEventListener("scroll",
							NicePayStd.niceLayerHandler, false);
				if (typeof window.attachEvent != "undefined")
					window.detachEvent("onscroll", NicePayStd.niceLayerHandler);
			}
		} catch (e) {
		}
	}
	...
	NicePayStd.uaMatch = function(ua) {
		...
		var match = /(edge)\/([\w.]+)/.exec(ua)
				...
				|| /(webkit)[ \/]([\w.]+).*(version)[ \/]([\w.]+).*(safari)[ \/]([\w.]+)/
						.exec(ua) || /(webkit)[ \/]([\w.]+)/.exec(ua)
				|| /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(ua)
				|| /(msie) ([\w.]+)/.exec(ua) || ua.indexOf("trident") >= 0
				&& /(rv)(?::| )([\w.]+)/.exec(ua)
		...
		// mobile browser
		if (browser.android || browser.bb || browser.blackberry || browser.ipad
				|| browser.iphone || browser.ipod || browser.kindle
				|| browser.playbook || browser.silk || browser["windows phone"]) {
			browser.mobile = true;
		}
...
<body>
	<button onclick="goPay(goPay)">123 </button>
